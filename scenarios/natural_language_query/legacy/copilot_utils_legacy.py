...
Plan your work and keep track of your progress in the action plan.
The action plan should look like this:
-----------------------------
- Overview: description of the overall plan
- Step 1: description of the first step. Status: Done
- Step 2: description of the second step. Status: Pending. Dependecy: Step 1
- Step 3: draw conclusion. Status: Pending. Dependecy: Step 1, Step 2
Note that never share the content of the notebook with the user. It's for your own reference only.
-----------------------------
## Action plan:
...
def execute_python_code(python_code, goal, execution_context):
    ...
    if 'data_from_display' in st.session_state: 
        data_display = st.session_state['data_from_display']
        data_display = "Warning: Only top 30 rows can be displayed. The table print below may not cover all underlying data. Use aggregate functions if needed \n"+str(data_display)
...
CODER_AVAILABLE_FUNCTIONS = {
            ...
            "retrieve_context": retrieve_context,
            "create_or_update_action_plan": create_or_update_action_plan,
        } 
...
CODER_FUNCTIONS_SPEC= [  
    
    {
        ...
        "description": "A special python interface that can run data analytical python code against the SQL database and data visualization with plotly. Data and visualization object can only be observed using display() function inside your code. Python's print function will not work. For dataframe in display() function, due to system limitation, only top 30 rows can be displayed. So don't rely on observing the entire dataframe if there are more than 30 rows.",
        "parameters": {
            "type": "object",
            "properties": {
                "python_code": {
    ...
    },
    {
        ...
        "name": "create_or_update_action_plan",
        "description": "Create new or update the current action plan with a new one.",
        "parameters": {
            "type": "object",
            "properties": {
                "new_or_updated_plan": {
                    "type": "string",
                    "description": "New or changed content of the action plan"
                },

            },
            "required": ["new_or_updated_plan"],
        },
    }},
