...
class Agent(BaseAgent):  
    ...
    async def chat_async(self, prompt: str) -> str:  
        ...
        try:  
            ...
            # Extract the final response - use the last non-handoff message
            assistant_response = None
            for i in range(len(response.messages)-1, -1, -1):
                msg = response.messages[i]
                content = str(getattr(msg, 'content', ''))
                
                # Skip function calls, empty messages, or handoff messages
                if (not content or 
                    content.startswith('[Function') or 
                    any(f'@{name}' in content for name in ["crm_billing", "product_promotions", "security_authentication"])):
                    continue
            ...
            # If no valid message found, use the last message
